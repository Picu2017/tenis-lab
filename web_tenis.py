...
# --- IMPORTACIÓN NIVEL INGENIERÍA ---
# Si 'import mediapipe.solutions' falla, intentamos la ruta directa al binario
try:
    import mediapipe as mp
    from mediapipe.python.solutions import pose as mp_pose
except ImportError:
    st.error("Error crítico: La librería MediaPipe no se instaló correctamente en el servidor.")
    st.info("Por favor, sigue el paso de 'Borrar y Recrear la App' que detallo abajo.")
    st.stop()
...
def load_pose_engine():
    # model_complexity=0 es el modelo 'Lite', no requiere descargas externas
    return mp_pose.Pose(

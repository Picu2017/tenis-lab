...
# Usamos model_complexity=0 porque ya viene incluido en la librería
# y no necesita descargar archivos externos (evita el PermissionError)
@st.cache_resource
...
# Conexiones biomecánicas de interés (13 puntos)
...
if uploaded_file is not None:
    ...
    while cap.isOpened() and run:
        ret, frame = cap.read()
        if not ret:
            cap.set(cv2.CAP_PROP_POS_FRAMES, 0) # Loop al terminar
            continue

        # Redimensionar para fluidez web (480px)

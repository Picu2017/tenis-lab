...
# Forzamos la carga de la solución de pose de forma directa
...
# Inicialización de IA (Ruta directa para evitar AttributeError)
...
if uploaded_file is not None:
    ...
    while cap.isOpened() and run:
        ...
        if results.pose_landmarks:
            ...
            puntos_dibujar = [0, 11, 12, 13, 14, 15, 16, 23, 24, 25, 26, 27, 28]
            for i in puntos_dibujar:
                p = lm[i]
                if p.visibility > 0.5:
                    cv2.circle(frame, (int(p.x*w), int(p.y*h)), 3, (0, 0, 255), -1)

            # Eje vertical de la cadera (Blanco)
            cx = int(lm[idx_c].x * w)
